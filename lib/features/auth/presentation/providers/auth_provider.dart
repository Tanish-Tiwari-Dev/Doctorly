...
class AuthState {
  const AuthState({
    this.user,
    this.guestModeExplicitlyChosen = false,
  ...
  final bool guestModeExplicitlyChosen;
  ...
  AuthStatus get status {
    ...
    if (user!.isAnonymous) {
      return guestModeExplicitlyChosen
          ? AuthStatus.guest
  ...
  AuthState copyWith({
    supabase.User? user,
    bool? guestModeExplicitlyChosen,
    bool? isLoading,
    bool? isSendingOtp,
    bool? isVerifyingOtp,
    ...
    return AuthState(
      user: user ?? this.user,
      guestModeExplicitlyChosen:
          guestModeExplicitlyChosen ?? this.guestModeExplicitlyChosen,
...
class AuthNotifier extends AsyncNotifier<AuthState> {
  ...
  Future<AuthState> build() async {
    ...
    final guestModeChosen = await authRepo.isGuestModeExplicitlyChosen();

    _subscription = authRepo.onAuthStateChange.listen((_) async {
      final user = authRepo.currentUser;
      final newUserId = user?.id;
      final isGuestChosen = await authRepo.isGuestModeExplicitlyChosen();
      ...
      state = AsyncValue.data(
        AuthState(
          user: user,
          guestModeExplicitlyChosen: isGuestChosen,
          isLoading: false,
          isSendingOtp: false,
    ...
    return AuthState(
      ...
      guestModeExplicitlyChosen: guestModeChosen,
      isLoading: false,
    );
  }
  ...
  /// Explicitly enters guest mode on user demand.
  Future<void> enterGuestMode() async {
    ...
    try {
      ...
      LoggerService.instance.log.severe('Guest sign-in failed', e, st);
      final updated = state.valueOrNull ?? const AuthState();
      state = AsyncValue.data(
        updated.copyWith(
          isSigningInGuest: false,
